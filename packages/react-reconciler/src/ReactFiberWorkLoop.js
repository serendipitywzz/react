...
// 正在处理中的fiber节点
// TODO workInProgress实际上是React的双缓冲技术(double buffering)
// TODO 这个过程中内存同时存在2棵树
// 其一：代表当前界面的fiber已经展示出来，挂载到fiberRoot.current上
// 如果是初次构造（初始化渲染），页面还没有渲染，此时界面对应的fiber树为u空
// fiberRoot.current = null
// 其二：正在构造的fiber树（即将展示出来挂载到HostRootFiber.alternate上
// 当构造完成之后，重新渲染页面，最后切换 fiberRoot.current = workInProgress, 使得fiberRoot.current重新指向代表当前界面的fiber树.
// Fiber树的构造过程就是ReactElement转为Fiber的过程
// fiberRoot和HostRootFiber包含渲染状态、优先级队列、指向根节点等是一个调度中心
// HostRootFiber当前Fiber树的根节点，连接调度中心（FiberRoot）和真实页面
// fiberRoot是一个大的容器
